...
void saveData(const char* HEALTHFILEPATH, const HealthData* health_data) {
	...
	/*cf. <cal_healthdata.h> 
		- [Exercise구조체의 멤버변수 수정된 상태] calories_burned_per_minute -> calories_burned (총 시간 소모칼로리 의미)*/
	 
    // ToCode: to save the chosen exercise and total calories burned 
    fprintf(file, "[Exercises] \n");
    	//1. HealthData 구조체 멤버 exercises배열에 저장된 정보를 for문으로 차례차례 'health_data.txt’ 파일로 백업(fprintf) 
    	//exercises[i] 구조체의 멤버 : i번째 운동의 이름, i번째 운동의 "총시간" 소모칼로리
    ...
    fprintf(file, "\n[Diets] \n");
		...
		//2. HealthData 구조체 멤버 diet배열에 저장된 정보를 for문으로 차례차례 'health_data.txt'파일로 백업(fprintf)
    ...
    // ToCode: to save the total remaining calrories
    	//3. 'health_data.txt’파일로 칼로리 백업(fprint)
    fprintf(file, "\n[Total] \n");
...
void printHealthData(const HealthData* health_data) {
	...
	printf("=========================== History of Exercise =======================\n");
		  ...
		  //1. HealthData 구조체 멤버 exercises배열에 저장된 정보를 for문으로 차례차례 print.
	...
	// 3. total calories burned, total calories intake, and the remaining calories print 
	...
	;
    		// 해당함수 내에서는 아무 처리안함. main.c에서 시스템 종료를 위해 do문으로 바로 넘겨질 예정.
			//do문에서 “You have consumed all your calories for today!”print 후  + 시스템 종료 처리될 예정. 
	 printf("=======================================================================\n");
}
